...
public class ImageCache extends Cache {
    ...
    private static final CompressFormat DEFAULT_COMPRESS_FORMAT = CompressFormat.PNG; // Compress

    private static final int DEFAULT_COMPRESS_QUALITY = 100; // Qualty
    ...
    public static ImageCache findOrCreateCache(WhoApplication app, ImageCacheParams cacheParams) {
        ImageCache imageCache = app.getImageCache();
        if (imageCache == null) {
            imageCache = new ImageCache(cacheParams);
            app.setImageCache(imageCache);
        }
        return imageCache;
    }
    ...
    public static class ImageCacheParams {
        ...
        public int compressQuality = DEFAULT_COMPRESS_QUALITY; // compress ѹ��
                                                               // quality ����
         ...
         * class. Eg. setting percent���ٷֱȣ� to 0.2 would set the memory cache
         * to one fifth of the device memory class. Throws
         ...
         */
        public void setMemCacheSizePercent(Context context, float percent) { // Percent�ٷֱ�
            if (percent < 0.05f || percent > 0.8f) {
                throw new IllegalArgumentException("setMemCacheSizePercent - percent must be "
                        + "between 0.05 and 0.8 (inclusive)");
            }
            memCacheSize = Math.round(percent * getMemoryClass(context) * 1024 * 1024); // round
                                                                                        // ��Բ�Σ���������
        }
